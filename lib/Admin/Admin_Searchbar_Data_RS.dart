...
class Admin_Searchbar_Data_RS extends StatefulWidget {
  const Admin_Searchbar_Data_RS({Key? key}) : super(key: key);

  @override
  _Admin_Searchbar_Data_RSState createState() => _Admin_Searchbar_Data_RSState();
}

class _Admin_Searchbar_Data_RSState extends State<Admin_Searchbar_Data_RS> {
  ...
  Future <List<Note>> fetchNotes() async{
    var url ="http:/$ip_address/Build_buddy_Php_files/Admin/Admin_View_Real_Estaters.php";
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(title: Text("Search Real Estaters",style: GoogleFonts.prompt(color: Colors.red.shade900),),
...
List<Note> RsModelFromJson(String str) => List<Note>.from(json.decode(str).map((x) => Note.fromJson(x)));
